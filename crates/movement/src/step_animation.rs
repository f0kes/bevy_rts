...
pub fn animate_steps(
    ...
    for (mut transform, mut step_animation, velocity) in query.iter_mut() {
        ...
        transform.translation.y -= step_animation.last_applied;
        ...
        if speed > 0.1 {
            ...
            let height_offset = (step_animation.phase.sin() + 1.0)
                * 0.5
                * step_animation.step_height;

            // Apply the vertical offset to the transform
            transform.translation.y += height_offset;
            step_animation.last_applied = height_offset;
        } else {
            step_animation.phase = 0.0;
            step_animation.last_applied = 0.0;
        }
    }
}
