...
type AnimationsWithPaths<T> = AnimationsCollection<T>;
type AnimationsWithHandles<T> = Vec<AnimationWithHandles<T>>;
...
pub struct AnimationLibrary<T: AnimationTypes> {
    pub animations: HashMap<AnimationKey<T>, AnimationClip>,
}
#[derive(Resource, Default)]
...
impl<T: AnimationTypes> AnimationLibrary<T> {
    ...
    pub fn get_animation(&self, key: &AnimationKey<T>) -> Option<&AnimationClip> {
        self.animations.get(key)
    }
    pub fn add_animation(&mut self, key: AnimationKey<T>, clip: AnimationClip) {
